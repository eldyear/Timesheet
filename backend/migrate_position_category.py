...
print("Checking table 'positions' for 'category' column...")

with engine.connect().execution_options(isolation_level="AUTOCOMMIT") as conn:
    if not column_exists(conn, "positions", "category"):
        conn.execute(text("""
            ALTER TABLE positions
            ADD COLUMN category INTEGER DEFAULT 99
        """))
        print("✓ 'category' column added to positions table")
    else:
        print("  (skip) 'category' column already exists in positions table")

print("\n✓ Position category migration complete!")
